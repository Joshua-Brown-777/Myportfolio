  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-br from-black via-purple-950/80 to-black rounded-xl border-2 border-purple-400/40 flex items-center justify-center overflow-hidden" style={{ transform: 'scaleX(-1)' }}>
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        <div className="h-24 w-1" style={{ background: 'linear-gradient(to bottom, #DC2626 0%, #9333EA 40%, #7C3AED 60%, #60A5FA 100%)' }} />
        <div className="text-8xl italic" style={{
          background: 'linear-gradient(135deg, #DC2626 0%, #9333EA 40%, #7C3AED 60%, #60A5FA 100%)',
          WebkitBackgroundClip: 'text',
          WebkitTextFillColor: 'transparent',
...
function CarouselCard({ song, position, isCenterCard, offset, index, isDragging }: CarouselCardProps) {
  ...
  return (
    ...
    >
      <div className="relative w-full h-[490px]" style={{ transformStyle: 'preserve-3d' }}>
        {/* Card Front */}
        <motion.div
        ...
        >
          ...
          >
            <div
              className="youtube-player w-full h-full relative"
              style={{
            ...
            >
              {/* Desktop Player */}
              <div className="hidden md:block w-full h-full" style={{ pointerEvents: 'auto' }}>
                <YouTubePlayer videoId={song.videoId} className="w-full h-full" />
              </div>
            
              {/* Mobile Player */}
              <div className="block md:hidden w-full h-full" style={{ pointerEvents: 'auto' }}>
                <YouTubePlayer videoId={song.videoId} className="w-full h-full" />
              </div>
            </div>
...
export function Projects() {
  ...
  const handleSeekLeft = () => {
    setCenterRotation((prev) => {
      ...
      return newValue < 0 ? songs.length - 1 : newValue;
    });
  };
  ...
  const handleSeekRight = () => {
    setCenterRotation((prev) => {
      const newValue = prev + 1;
      return newValue >= songs.length ? 0 : newValue;
    });
  };

  return (
    <section id="music" className="min-h-screen py-12 md:py-24 bg-gradient-to-b from-black via-purple-950/50 to-black flex items-center relative overflow-hidden">
      {/* Ambient orbs */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
        ...
        <div className="relative w-full h-[600px] md:h-[700px] flex items-center justify-center" style={{ perspective: '2000px' }}>
          ...
          {/* Seek Left Button - Desktop */}
          <button
            onClick={handleSeekLeft}
            className="hidden md:block absolute left-8 top-1/2 -translate-y-1/2 z-[60] bg-purple-600 hover:bg-purple-700 text-white rounded-full p-4 shadow-xl transition-all hover:scale-110"
            aria-label="Previous track"
          >
            <ChevronLeft className="w-6 h-6" />
          </button>

          {/* Seek Right Button - Desktop */}
          <button
            onClick={handleSeekRight}
            className="hidden md:block absolute right-8 top-1/2 -translate-y-1/2 z-[60] bg-purple-600 hover:bg-purple-700 text-white rounded-full p-4 shadow-xl transition-all hover:scale-110"
            aria-label="Next track"
          >
            <ChevronRight className="w-6 h-6" />
          </button>

          {/* Slider navigation with mobile seek buttons */}
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-4 w-full max-w-md px-4">
            {/* Seek Left Button - Mobile */}
            <button
              onClick={handleSeekLeft}
              className="md:hidden bg-purple-600 hover:bg-purple-700 text-white rounded-full p-3 shadow-xl transition-all hover:scale-110 flex-shrink-0"
              aria-label="Previous track"
            >
            ...
            {/* Slider */}
            <div className="flex-1" onPointerDown={handleSliderPointerDown}>
              <Slider
